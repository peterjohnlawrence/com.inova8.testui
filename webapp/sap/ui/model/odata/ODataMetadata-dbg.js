	...
	*/
	...
	ODataMetadata.prototype._getObjectMetadata = function(sObjectType, sObjectName, sNamespace) {
		...
		if (sObjectName && sNamespace) {
			...
			/**
			* now merge the inherited via Basetype properties, keys etc
			*/
			if(oObject.baseType){
				var aBaseEntityTypeName = this._splitName(oObject.baseType);
				var oBaseEntityType = this.mEntityTypes[aBaseEntityTypeName[0]];
				if(!oBaseEntityType){
					oBaseEntityType = this._getObjectMetadata("entityType", aBaseEntityTypeName[0], aBaseEntityTypeName[1]);
				}
				if (oBaseEntityType) {
					this.mEntityTypes[oBaseEntityType.name] = oBaseEntityType;
				}
				var oMergedObject = {};
				oMergedObject.baseType =oObject.baseType;
				oMergedObject.name =oObject.name;
				oMergedObject.namespace =oObject.namespace;
				oMergedObject.extensions =oObject.extensions;
				oMergedObject.key =  oBaseEntityType.key;
				oMergedObject.navigationProperty = this._mergeArrays(oBaseEntityType.navigationProperty,oObject.navigationProperty);
				oMergedObject.property = this._mergeArrays(oBaseEntityType.property,oObject.property);			
	...
	/**
	 * get the merged array from two arrays
	 */
	ODataMetadata.prototype._mergeArrays = function(a,b) {	
		var result=[];
